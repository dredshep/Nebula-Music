...
const YearPicker: React.FC<{ value: string, onChange: (val: string) => void }> = ({ value, onChange }) => {
    ...
    return (
        <div className="relative min-w-[120px]" ref={pickerRef}>
            ...
            {isOpen && (
                <div
                    className="absolute w-64 bg-neutral-900 border border-white/10 rounded-2xl shadow-float-3 z-50 overflow-hidden animate-scale-in"
                    style={dropdownStyle}
                >
                    ...
                    <div className="grid grid-cols-3 gap-2 p-3">
                        {years.map(y => (
                            <button
                                ...
                                className={`py-3 rounded-xl text-xs font-bold transition-all ${y === parseInt(value) ? 'bg-primary text-black shadow-glow-sm' : 'text-neutral-400 hover:bg-white/10 hover:text-white'} ${y > currentYear + 5 ? 'opacity-20 cursor-not-allowed' : ''}`}
                            >
                                {y}
                            </button>
                        ))}
...
const ViewHeader = ({ currentView, children }: { currentView: View, children?: React.ReactNode }) => (
    <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between mb-8 gap-6 animate-fade-in">
        <h2 className="text-4xl md:text-5xl font-black capitalize flex items-center tracking-tight text-white hidden md:flex">
    ...
    const PaginationControls = () => (
        <div className="flex justify-center items-center space-x-2 my-8">
            <button
                ...
                disabled={page === 0}
                className="flex items-center px-6 py-2.5 rounded-full bg-white/5 hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed transition border border-white/5 font-bold text-sm"
            >
                <ChevronLeft className="w-4 h-4 mr-2" /> Prev
            </button>

            <div className="px-6 font-mono text-sm text-neutral-400 bg-black/20 rounded-full py-2.5 border border-white/5 mx-2">
    ...
    return (
        <div className="p-6 md:p-12 min-h-full flex flex-col max-w-[1800px] mx-auto animate-fade-in">
            ...
            {isLoading && !displayItems.length ? (
                ...
                <>
                    {isSongView ? (
                        <div className="rounded-lg overflow-hidden flex-1 bg-neutral-900/50">
                            <div className="overflow-x-auto custom-scrollbar">
                                <table className="w-full text-left text-sm text-neutral-400">
                                    <thead className="bg-white/5 text-neutral-300 uppercase tracking-widest text-[10px] font-bold">
                                        <tr>
                                    ...
                                    <tbody className="divide-y divide-white/5">
                                        {displayItems.map((song: ISong, idx) => (
                                            <tr key={song.id} className="hover:bg-white/5 group transition-colors">
                                                ...
                                                <td className="p-4 font-bold text-white cursor-pointer" onClick={() => playSong(song, displayItems as ISong[])}>
                                                    <div className="flex items-center gap-4">
                                                        <img src={service.getCoverArtUrl(song.id, 50)} className="w-10 h-10 rounded object-cover bg-neutral-800 shadow group-hover:scale-105 transition-transform" loading="lazy" alt="" />
                                                ...
                                                <td className="p-4 hidden md:table-cell cursor-pointer hover:text-white transition-colors text-sm font-medium" onClick={(e) => { e.stopPropagation(); if (song.artistId) setView('ARTIST_DETAIL', song.artistId); }}>{song.artist}</td>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 content-start">
                            {displayItems.map((item: any, i) => (
                                ...
                                >
                                    <div className={`aspect-square overflow-hidden mb-3 relative shadow-lg bg-neutral-900 ${currentView === 'ARTISTS' ? 'rounded-full' : 'rounded-lg'}`}>
                                    ...
                                    <div className={currentView === 'ARTISTS' ? 'text-center' : ''}>
                                        <h3 className="font-semibold text-white truncate text-sm mb-1">{item.name}</h3>
                                        {currentView !== 'ARTISTS' && currentView !== 'PLAYLISTS' && <p className="text-xs text-white/40 truncate">{item.artist}</p>}
                                        {currentView === 'ARTISTS' && <p className="text-xs text-white/40">{item.albumCount} Albums</p>}
                                        {currentView === 'PLAYLISTS' && <p className="text-xs text-white/40">{item.songCount} Songs</p>}
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    {displayItems.length === 0 && !isLoading && (
                        <div className="text-center py-32 text-neutral-500 animate-fade-in">
                            ...
                            <button onClick={resetFilters} className="px-6 py-2.5 rounded-full bg-white/5 hover:bg-white/10 text-white font-bold text-sm transition border border-white/5">Clear all filters</button>
                        </div>
                    )}
                </>
            )}
