...
export const BrowseView: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    const mix2Songs = random.slice(20, 40);
    const mix3Songs = random.slice(40, 60);
    ...
    const createMix = (idSuffix: string, title: string, desc: string, icon: any, songs: ISong[]) => ({
        ...
        coverArt: songs[0]?.coverArt || songs[0]?.id, // Use first song art
        songs
    });

    setGeneratedMixes([
        createMix('flow', 'Flow State', 'Focus generated just for you', Zap, mix1Songs),
        createMix('oldies', 'Nostalgia Trip', 'Rediscover your old favorites', Radio, mix2Songs),
