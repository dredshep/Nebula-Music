...
import { Play, Clock, ArrowLeft, MoreVertical, Trash2, ListMusic, Shuffle, GripVertical } from 'lucide-react';
import { Visualizer } from '../components/Visualizer';

export const PlaylistDetailView: React.FC = () => {
  const { viewData, setView, playSong, isPlaying, queue, currentSongIndex, playlists, deletePlaylist, reorderPlaylist, service } = useStore();
  ...
  const onDragStart = (e: React.DragEvent, index: number) => {
      ...
      // Need to allow move effect
      e.dataTransfer.effectAllowed = "move";
      // Ghost image defaults are usually fine, or could setDragImage here
  };
    ...
    <div className="flex flex-col h-full pb-24">
      ...
      <div className="relative p-8 md:p-12 flex flex-col md:flex-row items-center md:items-end gap-10 overflow-hidden">
        ...
        <div className="relative z-10 flex-1 text-center md:text-left">
            <button onClick={() => setView('PLAYLISTS')} className="mb-4 flex items-center justify-center md:justify-start text-neutral-400 hover:text-white transition text-sm">
                <ArrowLeft className="w-4 h-4 mr-1" /> Back to Playlists
            </button>
                    ...
                    {playlist.songs?.map((song, idx) => {
                        ...
                        return (
                            <tr 
                                ...
                                className={`group transition-colors hover:bg-white/5 cursor-move ${isCurrent ? 'bg-white/10' : ''} ${isDragging ? 'opacity-30 bg-primary/20' : ''}`}
                            >
                                    ...
                                    <div className="flex items-center justify-center">
                                        {isCurrent && isPlaying ? (
                                            ...
                                            <div className="flex items-center justify-center w-6">
                                                ...
                                                <GripVertical className="w-4 h-4 hidden group-hover:block text-neutral-500 hover:text-white cursor-grab" />
                                            </div>
                                        )}
                                    </div>
                                </td>
                                ...
                                <td className="p-4 text-right">
                                    <button className="p-2 rounded-full hover:bg-white/10 text-neutral-500 hover:text-white transition">
