...
export const Player: React.FC = () => {
  ...
  }, [audioRef]); // Stable ref, runs once

  // Parse LRC Format Robustly
  const parseLyrics = useCallback((lrc: string) => {
      if (!lrc) return [];
      const lines = lrc.split('\n');
      const result: SyncedLine[] = [];
      
      // Matches [00:00.00] or [00:00.000] or [00:00]
      // \d+ allows for minutes > 99 (unlikely but possible)
      ...
      for (const line of lines) {
          ...
          if (matches.length > 0) {
              ...
              if (!text) continue; // Skip empty lines (optional, but helps with jumping)
  ...
  // Fetch lyrics when song changes or tab changes
  useEffect(() => {
    setSyncedLyrics([]); // Reset immediately to prevent old lyrics showing
    setLyrics('');
  ...
  // Auto-scroll synced lyrics (Only when index changes to prevent jitter)
  useEffect(() => {
  ...
  const renderQualityBadge = (suffix?: string, bitrate?: number, small = false) => {
      ...
      return (
          <div className={`flex items-center gap-1.5 ${small ? 'opacity-70' : ''}`}>
              <span className={`px-1 py-0.5 rounded text-[9px] font-bold border ${isLossless ? 'border-yellow-500 text-yellow-500 bg-yellow-500/10' : 'border-neutral-600 text-neutral-400 bg-neutral-800'}`}>
                  {s}
              </span>
              {bitrate && (
  ...
  const handleScrub = (e: React.ChangeEvent<HTMLInputElement>) => {
     ...
     setCurrentTime(newTime); // Immediate update for smooth drag
  };
    ...
    <>
      ...
      <div className={`fixed inset-0 z-50 flex flex-col bg-neutral-950 transition-transform duration-500 ease-in-out ${isExpanded || isZenMode ? 'translate-y-0' : 'translate-y-full'}`}>
        ...
        <div className={`relative z-10 flex-1 flex flex-col items-center px-4 md:px-8 w-full max-w-7xl mx-auto min-h-0 ${isZenMode ? 'hidden' : ''}`}>
            ...
            {activeTab === 'queue' && (
                <div className="w-full max-w-3xl flex-1 min-h-0 mb-8 bg-black/20 rounded-3xl border border-white/5 backdrop-blur-md flex flex-col overflow-hidden">
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-6">
                        {/* Anchor the queue header */}
                        <h3 className="text-xl font-bold text-white mb-4 sticky top-0 bg-neutral-900/90 backdrop-blur p-3 -mx-2 rounded-lg z-20 shadow-lg border-b border-white/5">
                            Up Next
                        </h3>
                                ...
                                >
                                    <div className="w-8 text-center text-sm font-mono mr-4">
                                        {idx === currentSongIndex && isPlaying ? (
                                            <div className="w-2 h-2 bg-primary rounded-full animate-ping mx-auto"></div>
                                        ) : (
                                    ...
                                    <div className="text-xs font-mono text-neutral-500 ml-4">
                        ...
                        <div className="min-h-full flex flex-col items-center justify-center py-32 px-6 md:px-16 text-center">
                            {loadingLyrics ? (
                                <div className="flex flex-col items-center">
                                    ...
                                    <span className="text-neutral-400">Fetching lyrics...</span>
                                </div>
                                    ...
                                    {syncedLyrics.map((line, i) => {
                                        ...
                                        return (
                                            <p 
                                                ...
                                                className={`transition-all duration-500 ease-in-out cursor-pointer origin-center leading-relaxed
                                                    ${isActive 
                                                        ? 'text-2xl md:text-4xl font-bold text-white scale-105 drop-shadow-[0_0_15px_rgba(255,255,255,0.5)] py-2' 
                                                        : 'text-lg md:text-2xl font-medium text-neutral-600 hover:text-neutral-300 blur-[0.5px] hover:blur-0 scale-100'}`}
                                                onClick={() => {
                                                    const audio = document.querySelector('audio');
                                ...
                                <div className="whitespace-pre-wrap leading-loose text-lg md:text-xl text-neutral-300">
            ...
            {/* MAIN CONTROLS (Bottom of expanded) */}
            <div className="w-full max-w-4xl bg-neutral-900/80 backdrop-blur-xl rounded-3xl border border-white/10 p-6 md:p-8 shadow-2xl mb-8">
                 ...
                 <div className="flex items-center w-full">
                     ...
                     <div className="flex-1 basis-0 min-w-0 flex items-center justify-start gap-4">
                          ...
                          <button onClick={(e) => { e.stopPropagation(); openPlaylistModal(currentSong); }} className="p-2 rounded-full hover:bg-white/10 text-neutral-400 hover:text-white transition">
                     ...
                     <div className="flex-none flex items-center justify-center gap-6 md:gap-8">
                          ...
                          <button onClick={nextSong} className="p-2 text-neutral-300 hover:text-white hover:scale-110 transition">
          ...
          <div className="flex flex-col items-center w-1/3">
              <div className="flex items-center gap-6 mb-2">
                  ...
                  <button 
                    ...
                    className="w-10 h-10 bg-white text-black rounded-full flex items-center justify-center hover:scale-105 hover:bg-primary hover:text-white transition shadow-lg"
                  >
                      {isPlaying ? <Pause className="w-4 h-4 fill-current" /> : <Play className="w-4 h-4 fill-current ml-0.5" />}
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); nextSong(); }} className="text-neutral-300 hover:text-white hover:scale-110 transition">
                      <SkipForward className="w-5 h-5 fill-current" />
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); setZenMode(true); }} className="hidden md:block text-neutral-500 hover:text-white transition" title="Zen Mode">
