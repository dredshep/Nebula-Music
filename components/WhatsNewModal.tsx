...
export const WhatsNewModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-fade-in">
      ...
      <div className="relative w-full max-w-lg bg-neutral-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-scale-in">

        {/* Header */}
        <div className="relative p-8 pb-6 overflow-hidden border-b border-white/5 bg-gradient-to-br from-neutral-800/50 to-neutral-900/50">
          ...
          <div className="relative z-10 flex flex-col items-center text-center">
            ...
            <div className="inline-flex items-center px-2.5 py-0.5 rounded-full bg-white/5 border border-white/10">
        ...
        {/* Content */}
        <div className="p-8 pt-6 flex-1 max-h-[60vh] overflow-y-auto custom-scrollbar bg-neutral-900/50">
          <h3 className="text-base font-semibold text-white mb-6 flex items-center justify-center">
            {currentLog.title}
          </h3>
          <ul className="space-y-4">
            {currentLog.changes.map((change, index) => (
              <li
                key={index}
                className={`flex items-start text-sm text-neutral-300 leading-relaxed animate-slide-up stagger-${Math.min(index + 1, 8)}`}
              >
